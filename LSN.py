...
def main(lambda_r,lambda_bc,lambda_ic,lambda_con,r,sigma):
    ...
    folder='./BigData_LSN_epochs{epochs}_r{rr}_sigma{sigma}_l1{l1}_l2{l2}_l3{l3}_l4{l4}'.format(
        epochs = args.epochs,
        rr=r,
        sigma=sigma,
        l1=lambda_r,
    ...
    for step in range(args.epochs):
        ...
        losses = (lambda_r * loss_r + lambda_bc * (loss_bc + loss_bc0) +
                  lambda_ic * loss_ic + lambda_con * loss_con)
        ...
        if step % args.write_step == 0:
            ...
            print('Epoch: %d, Time: %.2f, Loss_con: %.3e, Loss: %.3e,Loss_bc: %.3e, Loss_ic: %.3e, Loss_r: %.3e, Test error: %.3e, Relative Test error: %.3e, abs Test error: %.3e' %
                (step, elapsed, loss_con, losses, loss_bc, loss_ic, loss_r, test_error, relative_test_error, abs_test_error))
